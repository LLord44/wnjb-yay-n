...
client.on("ready", async () => {
  ...
  client.user.setActivity(`wn!oynat | discord.gg/Wuarten`, { type: "PLAYING" });
});
...
client.on("ready", () => console.log("Bot Aktif | NiviaGamer"));
...
client.on("message", async message => {
  ...
  if (cmd === "yttogether") {
    ...
    if (!channel || channel.type !== "voice")
      return message.channel.send("Geçersiz kanal belirtildi!");
    if (!channel.permissionsFor(message.guild.me).has("CREATE_INSTANT_INVITE"))
      return message.channel.send("CREATE_INSTANT_INVITE iznine ihtiyacım var");
    ...
    fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
      ...
      .then(invite => {
        if (invite.error || !invite.code)
          return message.channel.send("YouTube başlatılamadı!");
        message.channel.send(
          `**Yayını** Başlatmak İçin Tıklayın! ${channel.name}: <https://discord.gg/${invite.code}>`
        );
      })
      .catch(e => {
  ...
  if (cmd === "oynat") {
    ...
    if (!activity)
      return message.channel.send(
        `Doğru formatlar: \n${Object.keys(ACTIVITIES)
          .map(m => `- **${PREFIX}activity <Channel_ID> ${m}**`)
          .join("\n")}`
      );
    ...
    fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
      ...
      .then(invite => {
        if (invite.error || !invite.code)
          return message.channel.send(`Başlatılamadı **${activity.name}**!`);
        message.channel.send(
          `Başlatmak İçin Buraya Tıklayın! **${activity.name}** **${channel.name}**: <https://discord.gg/${invite.code}>`
        );
      })
      .catch(e => {
